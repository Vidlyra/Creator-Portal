...
/* ===========================
Dynamic Hero Banner (6s Interval)
===========================*/
...
function changeHero() {
  // Quotes around URL prevent spaces in filenames from breaking CSS
  hero.style.backgroundImage = `url('${featured[current].image}')`;
  heroTitle.textContent = featured[current].title;
  heroDescription.textContent = featured[current].description;
...
// Rotates text and background image every 6 seconds
...
document.querySelector(".original-next").addEventListener("click", () => {
  originalCarousel.scrollBy({ left: 360, behavior: "smooth" });
});
...
document.querySelector(".episode-next").addEventListener("click", () => {
  episodeCarousel.scrollBy({ left: 450, behavior: "smooth" });
});
